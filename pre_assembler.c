...
int isLanguageSavedWord(char *word) {
  ...
  for (i = 0; i < SAVED_WORDS_LEN; i++) {
    if (PreAssembler_CompareStrings_IgnoreSpace(
            word, saved_languages_words[i]) == 0) {
      return 1;
    }
  }
  return 0;
...
void PreAssembler_ReplaceExtension(char *filename, char *new_ext,
                                   char *new_filename) {
...
int PreAssembler_ReplaceMacros(const char *input_file,
  ...
  int num_macros = 0;
  int in_macro =
      0; /* flag to check if we are currently in a macro definition */
  FILE *input, *output;
  ...
  while (fgets(line, sizeof(line), input) != NULL) {
    ...
    if (strncmp(line, "mcr", 3) == 0) {
      ...
      for (i = 0; i < num_macros; i++) {
        if (PreAssembler_CompareStrings_IgnoreSpace(macros[num_macros].name,
                                                    macros[i].name) == 0) {
          printf(
              "[Error] macro name %s is already defined. problematic line: "
              "%d\n",
      ...
      while (word != NULL) {
        ...
        for (i = 0; i < num_macros; i++) {
          if (PreAssembler_CompareStrings_IgnoreSpace(word, macros[i].name) ==
              0) {
