...
/* is_current_word_empty -
 ...
 NOTE: The used of this function is in order to handle empty spaces or tabs
 between words in the line from the source file. */
int is_current_word_empty(char *word) {
  return strlen(word) == 0 ? TRUE : FALSE;
}

/* get_next_word -
 receives: a pointer to the line to take the word from, a pointer to the
 current_word, and the position in the line where last time the function was
 called returned. returns: the position where we stopped in the line. NOTE: in
 case of ":" or "." - these will be returned as part of the word. The reason is
 that these characters will assist with symbol type and commands recognition.
 They must be removed afterwards, in other function. NOTE2: The parameter
 last_position should be -1 when a new line is parsed. */
...
/* is_saved_word -
   receives: the current word to check if it is a saved word in the language or
   not. returns 1 if the word is a saved word, 0 if not. */
int is_saved_word(char *current_word, int line_number, int *syntax_errors,
  ...
  for (i = 0; i < NUM_OF_SAVED_WORDS; i++) {
    if (strcmp_lower(current_word, saved_languages_words[i]) == 0) {
      if (print_err) /*print error will also set syntax errors flag if a the
                        symbol is forbidden to be a saved word*/
      {
        printf(
            "Error in line %d - the word %s is a saved word in the language\n",
...
int is_command(char *curr_word) {
  if (strcmp_lower(curr_word, ".data") == 0 ||
      strcmp_lower(curr_word, ".string") == 0 ||
...
int is_symbol(char *current_word, int line_number, int *syntax_errors,
              int begining_of_sentence, int print_err) {
  ...
  if (!isalpha(current_word[0]) && !is_command(current_word)) {
    if (current_word[0] == '.') {
      ...
      fprintf(stderr,
              "Error in line %d - symbol cannot start with a non alphabetic "
              "character\n",
              line_number);
    }
    *syntax_errors = TRUE;
    ...
    if (length > 30) {
      fprintf(stderr,
              "Error in line %d - The word is too long to be a symbol\n",
              line_number);
      return FALSE;
    }
...
/* is_symbol_operand - operand which is not #<NUM> must start with an alphabetic
char (either symbol or r<NUM>) ONLY ERROR PRINT IS CHANGED IN THIS FUNC
receives: the current word to parse,
returns: 1 if it's a symbol, 0 if not. */
int is_symbol_operand(char *current_word, int line_number, int *syntax_errors,
                      int begining_of_sentence, int print_err) {
  size_t length = strlen(current_word);

  if (!isalpha(current_word[0]) && !is_command(current_word)) {
    fprintf(stderr,
            "Error in line %d - operand cannot start with a non alphabetic "
            "character\n",
            line_number);
    *syntax_errors = TRUE;
    return FALSE;
...
int is_store_command(char *current_word) {
  if (strcmp_lower(current_word, ".data") == 0 ||
      strcmp_lower(current_word, ".string") == 0) {
    memmove(current_word, current_word + 1,
            strlen(current_word) - 1); /* removes the "." from sentence. */
    current_word[strlen(current_word) - 1] =
        '\0'; /* in order to strcmp to be accurate */
    return TRUE;
  }
...
/* detect_store_command -
  ...
  If the current word matches to one of the store commands known, it is stored
  in the guidance_command variable of the sentence struct. */
...
int is_extern_or_entry_command(char *current_word) {
  if (strcmp_lower(current_word, ".extern") == 0 ||
      strcmp_lower(current_word, ".entry") == 0) {
    memmove(current_word, current_word + 1,
            strlen(current_word) - 1); /* removes '.' from the beginning */
    current_word[strlen(current_word) - 1] =
        '\0'; /* in order to strcmp to be accurate */
    return TRUE;
  }
...
/* get_next_member -
receives: a pointer to the temp member, the line to parse, line number, last
position in line and if we are expecting to a comma 1, or 0 if we don't. returns
the new position and edits the temp_member array with the current number
found.*/
int get_next_member(char *temp_member, char *line, int line_number,
                    int last_position, int *syntax_errors,
                    int *expecting_comma) {
  int i = 0;
  int new_position = skip_spaces(line, last_position);
  int number_ended =
      0; /*indication that no more digits are expected (for 3,4 4,5 example)*/
  ...
  while (line[new_position] != '\0' && line[new_position] != '\n' &&
    ...
    if (line[new_position] == '-' || line[new_position] == '+') {
      ...
      *expecting_comma =
          1; /* we can get a comma after getting at least 1 digit */
      temp_member[i] = line[new_position];
      i++;
      ...
      *syntax_errors = 1;
      ...
      fprintf(stderr,
              "Error in line %d - only integers can come after .data store "
              "command\n",
              line_number);
      *syntax_errors = 1;
    }
  ...
  if (is_current_word_empty(temp_member) && line[new_position] != '\0' &&
      line[new_position] != '\n' && line[new_position] != EOF)
    new_position =
        get_next_member(temp_member, line, line_number, new_position + 1,
                        syntax_errors, expecting_comma);

  return new_position;
}

/* verify_and_save_numbers -
receives: the sentence struct were the parse is stored, the last position of the
line, the line number and pointer to syntax errors. The function stores a number
(positive or negative) into the data array of the sentence struct and the number
of numbers stored. */
void verify_and_save_numbers(sentence *parsed, char *line, int last_position,
                             int line_number, int *syntax_errors) {
  char temp_member[MAX_DIGITS_SIZE]; /* The range of numbers to be coded is from
                                     -512 to 511. The most big array can include
                                     "-512'\0'" and thus, we want to define
                                     MAX_DIGITS_SIZE to 5. */
  int expecting_comma = 0;
  int new_position =
      skip_spaces(line, last_position); /* initializes position */
  int j = 0;
  int number;
  ...
  while (line[new_position] != '\0' && line[new_position] != '\n' &&
         ...
         line[new_position] != '\t') {
    new_position = get_next_member(temp_member, line, line_number, new_position,
                                   syntax_errors, &expecting_comma);
    number = atoi(temp_member);
    if (number > 511 || number < -512) {
      fprintf(stderr,
              "Error in line %d - the range of numbers that can be translated "
              "with assembler that works with 10 bits is from -512 to 511. "
              "Number %d cannot be stored.\n",
              line_number, number);
      *syntax_errors = 1;
    } else {
  ...
  /*if expecting_comma == 0 it means that the last member encountered was a
  comma, or when we don't have any numbers. We reach to this "if" when the line
  ends and the data isn't completed */
...
/* verify_and_save_string -
   receives: the sentence struct, the line to parse, the last position in the
   line, the line number and a pointer to syntax errors. The function parses a
   string and saves it to sentence->string */
void verify_and_save_string(sentence *parsed, char *line, int last_position,
                            int line_number, int *syntax_errors) {
  ...
  */
  ...
  if (curr_char != '\"') {
    fprintf(stderr,
            "Error in line %d - '\"' expected at the beginning of the string\n",
            line_number);
    *syntax_errors = 1;
  }
  ...
  if (curr_char != '\"') {
    fprintf(stderr,
            "Error in line %d - '\"' expected at the end of the string\n",
            line_number);
    *syntax_errors = 1;
  }
  if (curr_char == '\0' && curr_char == EOF && curr_char == '\n') return;
  ...
  if (curr_char != '\0' && curr_char != EOF && curr_char != '\n') {
    fprintf(stderr,
            "Error in line %d -  unexpected characters after string ended\n",
            line_number);
    *syntax_errors = 1;
  }
...
/* parse_data_by_its_type -
   receives: the current sentence struct, the current word and the last
   position. it routes to the relevant function for parsing according the
   guidance command detected and stored in the guidance command variable in the
   sentence struct. */
void parse_data_by_its_type(sentence *parsed, char *line, int last_position,
                            int line_number, int *syntax_errors) {
  if (parsed->guidance_command == NUM)
    verify_and_save_numbers(parsed, line, last_position, line_number,
                            syntax_errors);
  else if (parsed->guidance_command == STRING)
    verify_and_save_string(parsed, line, last_position, line_number,
                           syntax_errors);
  return;
}
...
/* get_next_operand -
   receives: a pointer to current word, the line being parsed and the last
   position in the line. The function copies into current word the word that is
   supposed to be an operand. Verification for operand is doing in other
   functions. returns: the position where the function ended to read.*/
...
/* detect_operand - */
void detect_operand(char operand_position, sentence *parsed, char *temp_word,
                    int line_number, int *syntax_errors,
                    int *operands_in_sentence, int *temp_operand_type) {
  if (temp_word[0] == '#') {
    *temp_operand_type = 0;
    *operands_in_sentence = *operands_in_sentence + 1;
    ...
    if (!atoi(temp_word) && strcmp(temp_word, "0") != 0) {
      fprintf(stderr,
              "Error in line %d - the value after an immediate operand must be "
              "a number\n",
              line_number);
      return;
    } else if (operand_position == 'a') /* if we are checking the operand in the
                                           1st place after the opcode */
    {
  ...
  else if (is_symbol_operand(temp_word, line_number, syntax_errors, FALSE,
                             FALSE)) {
    *temp_operand_type = 1;
    *operands_in_sentence = *operands_in_sentence + 1;
...
/* check_destination_address_type -
    ...
    The function checks over the operands in the opcodes table that the type is
   one of it's destination address types. returns: true if supported, false if
   not.
    */
int check_destination_address_type(int ext_iteration, int op_type) {
  int j;

  for (j = 0; opcodes_table[ext_iteration].destination_operand_types[j] != -1;
       j++) {
...
/* validate_operand_for_opcode -
    receives: the sentence struct, the type of operand in source, the type of
   operand in destination (-999 should be passed if empty), the line number and
   a pointer of type int to syntax error. The function validates the operand
   according the definitions in opcodes_table in structs.c. If the qty_of_ops is
   equal to the ammount of operands that the opcode supports, If op_a is equal
   to one of the supported operands types for source, if op_b is equal to one of
   the operands types for destination - the function returns TRUE. Else, it
   returns FALSE. */
int validate_operand_for_opcode(sentence *parsed, int op_a, int op_b,
                                int qty_of_ops, int line_number,
                                int *syntax_errors) {
  int i;
  int operand_error = 0;
  ...
  for (i = 0; i < NUM_OF_OPCODES; i++) {
    if (strcmp_lower(parsed->opcode, opcodes_table[i].opcode) == 0) {
      if (opcodes_table[i].qty_of_supported_operands < qty_of_ops) {
        fprintf(stderr,
                "Error in line %d - too many operands for the opcode %s\n",
      ...
      if (qty_of_ops == 1) /* if there is a single operand, it is categorized as
                              destination operand */
      {
        if (check_destination_address_type(i, op_a) == FALSE) {
          fprintf(stderr,
                  "Error in line %d - the address type of the destination "
                  "operand doesn't match to the opcode %s\n",
                  line_number, parsed->opcode);
          return FALSE;
        }
        ...
        if (check_source_address_type(i, op_a) == FALSE) {
          fprintf(stderr,
                  "Error in line %d - the address type of the source operand "
                  "doesn't match to the opcode %s\n",
                  line_number, parsed->opcode);
          *syntax_errors = 1;
          operand_error = 1;
        }
...
int get_jump_operands(sentence *parsed, char *line_with_offset,
                      int *syntax_errors) {
  int i = 0, j = 0;
  char *op_a = (char *)calloc(strlen(line_with_offset) + 1, sizeof(char));
  char *op_b = (char *)calloc(strlen(line_with_offset) + 1, sizeof(char));
  char *op_direct = (char *)calloc(strlen(line_with_offset) + 1, sizeof(char));
...
/* verify_operands -
  receives: the struct of sentence type, the last position in the line, the line
  number and a pointer to syntax errors. The function goes over the operands,
  check their type, validate they match the opcode and parse them into the
  sentence struct if legal. (Note: parsed->opcode holds the opcode already
  found).
  */
void verify_operands(sentence *parsed, char *line, int last_position,
                     int line_number, int *syntax_errors) {
  ...
  if (!parsed->is_jump) {
    ...
    if (is_current_word_empty(temp_word)) {
      validate_operand_for_opcode(parsed, -999, -999, operands_in_sentence,
                                  line_number, syntax_errors);
      return;
    }

    /*detect 1st operand */

    detect_operand('a', parsed, temp_word, line_number, syntax_errors,
                   &operands_in_sentence, &temp_operand_type_a);
    ...
    if (line[new_position] == ',') {
      ...
      /* check for 2nd operand, increase operands_in_sentence if found other
       * operand */
      detect_operand('b', parsed, temp_word, line_number, syntax_errors,
                     &operands_in_sentence, &temp_operand_type_b);
    }

    if (line[new_position] != '\n' && line[new_position] != EOF &&
        line[new_position] != '\0') {
      fprintf(stderr,
              "Error in line %d - end of line expected after matching number "
              "of operands\n",
              line_number);
      return;
    }
  ...
  if (operands_in_sentence == 1) /* if only a single operand was found, the
                                    operand is stored as destination */
  {
    if (parsed->is_jump) {
      return;
    }
    strcpy(
        ...
        parsed
            ->source_operand_type); /* 3 places so '\0' can be added - miunim */
    strcpy(parsed->source_operand_type, "");
    strcpy(parsed->operand_2,
           parsed->operand_1); /* for variables, registers, matrixes */
    strcpy(parsed->operand_1, "");
    parsed->immediate_operand_b =
        parsed->immediate_operand_a; /* when we have "#" */
    if (validate_operand_for_opcode(parsed, temp_operand_type_a, -999,
                                    operands_in_sentence, line_number,
                                    syntax_errors))
      convert_dec_to_x_bit_binary(
          temp_operand_type_a, 3,
          parsed->dest_operand_type); /* VERIFY THE FUNCTION IS CORRECT */
    else
      return;
  }

  else if (operands_in_sentence == 2 &&
           (validate_operand_for_opcode(
               parsed, temp_operand_type_a, temp_operand_type_b,
               operands_in_sentence, line_number, syntax_errors))) {
    convert_dec_to_x_bit_binary(temp_operand_type_a, 3,
                                parsed->source_operand_type);
...
/* init_sentece -
   ...
   The function initiates all values to 0, empty or null in order to avoid
   garbage in fields. */
...
int has_jump_command(char *line) {
  return strstr(line, "jmp") != NULL || strstr(line, "bne") != NULL ||
         strstr(line, "jsr") != NULL;
}

/*parse_sentence -
   receives: the line to be parsed and the current line number.
   The functions parses the line and populates the sentence structure (defined
   in data_structures.h) with the relevant fields. returns: a sentence
   structure. */
sentence *parse_sentence(char *line, int line_number, int *syntax_errors) {
  ...
  if (line_is_empty(line)) return NULL;

  init_sentence(parsed);

  last_position = get_next_word(current_word, line, -1);
  if (is_symbol(current_word, line_number, syntax_errors, TRUE,
                TRUE)) { /* if so, set is_symbol to 1, copy it to current_symbol
                   and get the next word into current_word */
  ...
  if (is_store_command(current_word)) {
    ...
    parse_data_by_its_type(parsed, line, last_position, line_number,
                           syntax_errors);
  }
    ...
    if (is_current_word_empty(current_word)) {
      fprintf(stderr,
              "Error in line %d - missing symbol after guidance command\n",
              line_number);
      *syntax_errors = TRUE;
      return parsed;
    } else if (is_symbol(current_word, line_number, syntax_errors, FALSE,
                         TRUE)) {
      if (parsed->is_symbol == 1)
        fprintf(stderr,
                "Warning in line %d - the line has both symbol and extern "
                "declaration\n",
                line_number);
      strcpy(parsed->symbol, current_word);
    }
  ...
  /* if first word is not symbol||store_cmd||extern or if first is symbol and
   * second is not store_cmd||extern:*/
