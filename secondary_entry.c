...
void add_item_to_code_list(
    memory_word *item) { /* Code list is a linked list of memory words */
...
symbol_line *get_symbol_line_from_symbol_table(char *symbol) {
  ...
  fprintf(stderr,
          "ERROR: Operand %s is not defined in symbol table (line %d)\n",
          symbol, line_number);
  is_error_found = 1;
  return NULL;
}
...
/* Encode registers into code table. operand_type should be one of source,
 destination or both. If both is specified, both source and destination
 registers will be encoded into the same memory word */
void encode_register(char *register_names[], char *operand_type) {
  ...
  if (strcmp(operand_type, "source") == 0) {
    ...
    } else if (strcmp(operand_type, "both") ==
               0) { /* encode both source and destination operands */
      strcpy(new_memory_word->bits, register1_val);
      strcat(new_memory_word->bits, register2_val);
    }
  }
...
void encode_instruction(sentence *curr) {
  ...
  strcat(new_memory_word->bits,
         ABSOLUTE_ARE); /* Instruction is always type absolute */
  add_item_to_code_list(new_memory_word);
  ic_second_pass++;
}

/* Will update memory addresses of data items in data tables to the addresses
 * after the code part in the code table. In addition will connect the last node
 * in the code linked list to the first node of the data linked list */
...
/* Will append to extern file and encode external operand into code table if
operand is external, otherwise encode as direct operand. */
...
void create_object_file() {
  ...
  convert_dec_to_base_2_special(IC - CODE_TABLE_START_ADDRESS,
                                conversion_result_2);
...
void combine_filename_with_new_file_extension(char *filename,
...
void ExecuteSecondEntry(char *filename) {
  ...
  combine_filename_with_new_file_extension(input_filename, object_filename,
                                           OBJECT_FILE_EXTENSION);
  combine_filename_with_new_file_extension(input_filename, extern_filename,
                                           EXTERN_FILE_EXTENSION);
  combine_filename_with_new_file_extension(input_filename, intern_filename,
  ...
  while (current_sentence) {
    if (current_sentence->is_action == 0) { /* is guidance sentence */
      if (current_sentence->guidance_command == ENTRY) {
        char conversion_result[BASE_2_SPECIAL_NUMBER_MAX_DIGITS];
        symbol_line *sl =
            get_symbol_line_from_symbol_table(current_sentence->symbol);
        if (!sl) {
          break;
        }
        convert_dec_to_base_2_special(sl->address, conversion_result);
        add_line_to_file(intern_fd, current_sentence->symbol,
                         conversion_result);
      }
  ...
  if (is_error_found) {
    fprintf(stderr,
            "ERROR: Errors found in second pass. Output files will not be "
            "created.\n");
